...
def predictNoCamera(X_frame, knn_clf=None, model_path=None, distance_threshold=0.4):
    """
    Recognizes faces in given image using a trained KNN classifier
    :param X_frame: frame to do the prediction on.
    :param knn_clf: (optional) a knn classifier object. if not specified, model_save_path must be specified.
...
def convert_image(frame,isCheckIn,isSelected,emit):
    ...
    if  process_this_frame % 60 == 0:
        ...
        frame = show_prediction_labels_on_image(frame, isCheckIn, predictions,isSelected, emit)
    return frame;


def show_prediction_labels_on_image(frame,isCheckIn, predictions,isSelected,emit):
    """
    ...
    for name, (top, right, bottom, left) in predictions:
        ...
        controller.insert(name,opencvimage,isCheckIn,emit);
...
def show_prediction_labels_on_image(frame, predictions, name_status):
    """
    Shows the face recognition results visually.
    :param frame: frame to show the predictions on
    :param predictions: results of the predict function
    ...
    for name, (top, right, bottom, left) in predictions:
        ...
        opencvimage = np.array(pil_image)
        # controller = EmployeeHistoryController();
        # controller.insert(name,opencvimage);
        if name not in name_status:
            # Nếu chưa có, đặt trạng thái ban đầu là False
            name_status[name] = False

        if name_status[name] is False:
            ...
            dateStr = current_datetime.strftime('%Y_%m_%d')
            timeStr = current_datetime.strftime('%H_%M_%S_%f')
            ...
            arr = []
            cv2.imwrite(url, opencvimage,[int(cv2.IMWRITE_JPEG_QUALITY), 90])

            api = Api();
            result = api.upload('upload/image', url);
...
def write_data(frame,isCheckIn,emit):
    if frame.shape[0] > 0 and frame.shape[1] > 0:
        img = cv2.resize(frame, (0, 0), fx=0.5, fy=0.5)
    else:
        print("Invalid frame dimensions.")
    ...
    if  process_this_frame % 60 == 0:
        imgImageNew = copy.deepcopy(img);
        predictions = predictNoCamera(imgImageNew, model_path="trained_knn_model.clf")
        search_prediction(frame, isCheckIn, predictions, emit)

def search_prediction(frame,isCheckIn, predictions,emit):
    pil_image = Image.fromarray(frame)
    draw = ImageDraw.Draw(pil_image)
    opencvimage = np.array(pil_image)
    for name, (top, right, bottom, left) in predictions:
...
if __name__ == "__main__":
    ...
    if time.time() % 30 == 0:
        name_status = {name: False for name in name_status}
    while 1 > 0:
        ret, frame = cap.read()
        if ret:
            ...
            if process_this_frame % 30 == 0:
                predictions = predict(img, model_path="trained_knn_model.clf")
            frame = show_prediction_labels_on_image(frame, predictions, name_status)
